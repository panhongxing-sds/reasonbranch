...
_DEFAULT_TOKENS = _REPO / "outputs" / "vsignal" / "tokens.jsonl"
...
def main():
    ...
    ap.add_argument("--tokens", type=Path, default=_DEFAULT_TOKENS)
    args = ap.parse_args()
    rows = [json.loads(l) for l in args.tokens.open() if l.strip()]
