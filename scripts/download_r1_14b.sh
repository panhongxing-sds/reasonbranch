#!/usr/bin/env bash
# Download official DeepSeek-R1-Distill-Qwen-14B (bf16, ~28GB) via hf-mirror,
# with resume + retry until all index shards present.
set -uo pipefail
export HF_ENDPOINT="${HF_ENDPOINT:-https://hf-mirror.com}"
DEST="${DEST:-/mnt/afs/L202500372/specreason/models/DeepSeek-R1-Distill-Qwen-14B}"
REPO="${REPO:-deepseek-ai/DeepSeek-R1-Distill-Qwen-14B}"
...
try:
    snapshot_download(
        repo_id=repo, local_dir=dest, max_workers=6,
